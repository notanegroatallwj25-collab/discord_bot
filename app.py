...
async def on_ready():
    ...
    try:
        if GUILD_ID:
            # مزامنة مع سيرفر محدد (تظهر الأوامر فوراً)
            guild = discord.Object(id=GUILD_ID)
            synced = await bot.tree.sync(guild=guild)
            print(f"✅ تم تفعيل {len(synced)} أمر(أمر) سلاش في السيرفر {GUILD_ID}")
        else:
            # مزامنة عامة (قد تستغرق حتى ساعة لتظهر في كل السيرفرات)
        ...
        try:
            ...
            if "Success. Downloaded item" in output:
                try:
                    ...
                    download_path = f"C:\\SteamCMD\\steamapps\\workshop\\content\\431960\\{workshop_id}"
                ...
                embed = discord.Embed(
                    ...
                    description="تأكد من صحة اسم المستخدم وكلمة المرور في ملف `.env`.",
                    color=discord.Color.red()
                )
                await interaction.followup.send(embed=embed)
